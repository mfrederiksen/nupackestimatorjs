const defaultRoundingFn = require('./round').roundStandard;

/***
 * Calculates Nupack estimates.
 */
class EstimateCalculator {
   ...
   */
  constructor(flatRate, labourRate, materialsRateMap = {}, roundingFn) {
    ...
    this.roundingFn = roundingFn || defaultRoundingFn;
  }
  ...
  _calcLabourMarkup(baseMarkup, peopleRequired) {
    if (peopleRequired < 0) throw new RangeError('peopleRequired must be >= 0');
  ...
  _calcMaterialsMarkup(baseMarkup, materialsType) {
    const materialsMarkupRate = this.materialsRateMap[materialsType.toLowerCase()] || 0;
    if (typeof materialsMarkupRate !== 'number') throw new TypeError(`Markup rate for '${materialsType.toLowerCase()}' must be a number.`);
  ...
  calculate(basePrice, peopleRequired, materialsType) {
    const baseMarkup = this._calcBaseMarkup(basePrice);
    const labourMarkup = this._calcLabourMarkup(baseMarkup, peopleRequired);
    const materialsMarkup = this._calcMaterialsMarkup(baseMarkup, materialsType);
    const finalEstimate = baseMarkup + labourMarkup + materialsMarkup;

    // Round to 2 decimal points
    return this.roundingFn(finalEstimate);
  }
}
